...
const matchCommander = html => {
  ...
  const table = html.match(
    /<font size="?\+1"?>Banlist<\/font>[\s\S]+?<table width="?100%"?>([\s\S]+?)<\/table>/i,
  );
  if (table === null) {
  ...
  let cards = table[1].match(
    /<li><a .*?href="http:\/\/deckbox\.org\/mtg\/.+?">.+?<\/a>/g,
  );
  if (cards === null) {
  ...
  // Fix Moxen.
  const moxen = cards.findIndex(
    card => card === 'Mox Sapphire, Ruby, Pearl, Emerald and Jet',
  );
  if (moxen !== -1) {
